...
const AdminSidebar = () => {
  ...
  return (
    <div className='h-[calc(100vh-64px)] md:flelx flex-col items-center pt-8 max-w-1/3
      ...
      <div className='w-full'>
        {adminNavlinks.map((link, index) => (
          <NavLink 
            key={index} 
            to={link.path} end
            className={({ isActive }) => `relative flex items-center max:md:justify-center gap-2
                                          w-full py-2.5 min-md:pl-10 first:mt-6 text-gray-400 
                                          ${isActive && 'bg-primary/15 text-primary group'}`}>
            {({ isActive }) => (
              <>
                <link.icon className='w-5 h-5'/>
                <p className='max-hd:hidden'>{link.name}</p>
